...
describe 'chef-cloudflarebeat::default' do
...
describe 'chef-cloudflarebeat::default' do
  ...
  context 'when the test is basic' do
    ...
    it 'includes another recipe' do
      expect(chef_run).to include_recipe('chef-cloudflarebeat::server')
    end
  ...
  context 'when the test is more complex' do
    let(:chef_run) do
      ...
      run.node.normal['chef-cloudflarebeat']['foo'] = :bar # Your attributes
